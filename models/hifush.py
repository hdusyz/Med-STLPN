...
class main_model(nn.Module):
    def __init__(self, num_classes, patch_size=(2, 2, 2), in_chans=1, embed_dim=96, depths=(2, 2, 2, 2),
                 num_heads=(3, 6, 12, 24), window_size=(1, 7, 7), qkv_bias=True, drop_rate=0,
        ...
        self.fu1 = HFF_block3D(ch_1=96, ch_2=96, r_2=16, ch_int=96, ch_out=96, drop_rate=HFF_dp)
        self.fu2 = HFF_block3D(ch_1=192, ch_2=192, r_2=16, ch_int=192, ch_out=192, drop_rate=HFF_dp)
        self.fu3 = HFF_block3D(ch_1=384, ch_2=384, r_2=16, ch_int=384, ch_out=384, drop_rate=HFF_dp)
        self.fu4 = HFF_block3D(ch_1=768, ch_2=768, r_2=16, ch_int=768, ch_out=768, drop_rate=HFF_dp)
    ...
    def forward(self, imgs):
       ...
       # print(x_s_4.shape)
        ...
        x_f_4 = self.fu4(x_c_4, x_s_4, x_f_3)
        #print(x_f_4.shape)
        x_fu = self.conv_norm(x_f_4.mean([-3, -2, -1]))  # global average pooling, (N, C, D, H, W) -> (N, C)
        x_fu = self.conv_head(x_fu)
...
class HFF_block2D(nn.Module):
    def __init__(self, ch_1, ch_2, r_2, ch_int, ch_out, drop_rate=0.):
        super(HFF_block2D, self).__init__()
...
class IRMLP2D(nn.Module):
    def __init__(self, inp_dim, out_dim):
        super(IRMLP2D, self).__init__()
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        """
        # [batch_size*num_windows, Md*Mh*Mw, total_embed_dim]
        B_, N, C = x.shape
...
class Global_block2D(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        # partition windows
        x_windows = window_partition2D(shifted_x, self.window_size)  # [nW*B, Mh, Mw, C]
        ...
        shifted_x = window_reverse2D(attn_windows, self.window_size, Hp, Wp)  # [B, H', W', C]
...
class BasicLayer2D(nn.Module):
    ...
    def create_mask(self, x, H, W):
        ...
        mask_windows = window_partition2D(img_mask, self.window_size)  # [nW, Mh, Mw, 1]
...
def window_partition2D(x, window_size: int):
    """
    Args:
        x: (B, H, W, C)
...
def window_reverse2D(windows, window_size: int, H: int, W: int):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size(M)
...
if __name__ == '__main__':
    # imgs = torch.randn(4, 1, 224, 224)
    imgs=torch.randn(4, 1, 16, 64, 64)
    model = HiFuse_Small(num_classes=2)
    output = model(imgs)
    print(output.shape)