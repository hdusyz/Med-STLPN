...
class Med_STLPN_seg(nn.Module):
    def __init__(self, img_shape=(16, 512, 512), input_dim=1, output_dim=1, embed_dim=768, patch_size=(16,128,128), num_heads=4, dropout=0.1):
        super().__init__()
        ...
        self.CMAF = CMFA(img_dim=768,tab_dim=768,hid_dim=768)
        self.stdm12 = STDM(768)
        self.stdm9 = STDM(512)
        self.stdm6 = STDM(256)
        self.stdm3 = STDM(128)
        self.stdm0 = STDM(64)
    def forward(self, x, return_clsfeat=False):
        #T0 branch
        ...
        z0 = self.decoder0(z0)
        #z0 = self.stdm0(z0)
        segout = self.decoder0_header(torch.cat([z0, z3], dim=1))
        #class
        out = self.avg_pool(clsfeat)
        out = out.view(out.size(0), -1)
        clsout = self.clshead(out)

        if return_clsfeat:
            return segout, clsout, clsfeat
        else:
