...
class Runtime_Observer_5fold:
    def __init__(self, log_dir, fold_idx=1, device='cuda', **kwargs):
        """
        The Observer of training, which contains a log file(.txt), computing tools(torchmetrics), and tensorboard writer.
        :author windbell
        :param log_dir: Output directory for logs and summaries.
        :param fold_idx: The current fold index (1-based).
        :param device: Default to 'cuda'.
        :param kwargs: Contains the experiment name and random number seed.
        """
        # 初始化最佳记录的字典
        ...
        # 日志文件目录和路径设置
        self.log_dir = os.path.join(str(log_dir), f"fold_{fold_idx}")
        os.makedirs(self.log_dir, exist_ok=True)
        self.log_ptr = open(os.path.join(self.log_dir, f'fold_{fold_idx}_log.txt'), 'w')
        ...
        if _kwargs['checkpoints_dir'] is not None:
            self.checkpoints_dir = os.path.join(_kwargs['checkpoints_dir'], f"fold_{fold_idx}")
        ...
        self.summary = SummaryWriter(log_dir=os.path.join(self.log_dir, 'summery'))

        # 日志初始化
        self.log_ptr.write(f'exp: {_kwargs["name"]}, seed: {_kwargs["seed"]}\n')
        self.early_stopping = EarlyStopping(patience=130, verbose=True)
    ...
    def excute(self, epoch):
        ...
        log_info = (
            f"-------\nEpoch {epoch + 1}:\n"
            f"Val Accuracy: {total_acc * 100:.2f}% || "
            f"Best Accuracy: {self.best_dicts['acc'] * 100:.2f}% "
            f"@epoch {self.best_dicts['epoch'] + 1}\n"
        )
        self.log(log_info)

        return self.early_stopping.early_stop

    def record(self, epoch, train_loss, val_loss, fold=None):
    # 如果传入 fold，则在日志中添加折次信息
        fold_info = f"Fold {fold}, " if fold is not None else ""
    
        self.summary.add_scalar('train_loss', train_loss, epoch)
        self.summary.add_scalar('val_loss', val_loss, epoch)

        self.log(f"{fold_info}Epoch {epoch + 1}, Average train Loss: {train_loss}\n"
             + f'Average val Loss: {val_loss}')
    
    def record_loss(self, epoch, loss, tloss, fold=None):
    # 如果传入 fold，则在日志中添加折次信息
        fold_info = f"Fold {fold}, " if fold is not None else ""
        ...
        self.log(f"{fold_info}Epoch {epoch + 1}, Training Loss: {loss}, Testing Loss: {tloss}")
    ...
    def finish(self):
        finish_info = (
            f"---experiment ended---\n"
            f"Best Epoch {self.best_dicts['epoch'] + 1}:\n"
            f"Accuracy: {self.best_dicts['acc'] * 100:.2f}%\n"
            f"Precision: {self.best_dicts['p'] * 100:.2f}%\n"
            f"F1 score: {self.best_dicts['f1'] * 100:.2f}%\n"
            f"AUC: {self.best_dicts['auc'] * 100:.2f}%\n"
            f"Recall: {self.best_dicts['recall'] * 100:.2f}%\n"
            "Exiting...\n"
        )
        self.log(finish_info)
        self.log_ptr.close()
